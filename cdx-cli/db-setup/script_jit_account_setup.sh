...
SECRET_NAME="CDX_SECRETS_v1.0.4"
...
# log "Creating ECS-SSM-Role..."       # will be done in permission set

# Create role and attach policies
# aws iam create-role \
#     --role-name ECS-SSM-Role \
#     --assume-role-policy-document file://trust-policy-ECS-SSM-ROLE.json

# aws iam create-policy \
#     --policy-name SSMSessionAndCommandPolicy \
#     --policy-document file://ssm-session-policy.json

# aws iam create-policy \
#     --policy-name ECSDescribeAndListTasksServices \
#     --policy-document file://ecs-describe-policy.json

# SSM_POLICY_ARN=$(aws iam list-policies --query 'Policies[?PolicyName==`SSMSessionAndCommandPolicy`].Arn' --output text)
# ECS_POLICY_ARN=$(aws iam list-policies --query 'Policies[?PolicyName==`ECSDescribeAndListTasksServices`].Arn' --output text)
...
# Configure additional security group rules
log "Configuring additional security group rules..."
