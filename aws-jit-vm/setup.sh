...
if [[ ${#ACCOUNTS_NEEDED[@]} -eq 0 ]]; then
    ...
    for acct_context in "${ACCOUNTS_NEEDED[@]}"; do
        ...
        while true; do
            ...
            # line (export KEY="value") or a bare value. Strips the "export KEY="
            # prefix, surrounding quotes, and trailing whitespace/CR.
            _read_cred_field() {
                local _label="$1" _val=""
                read -erp "  $_label: " _val
                # Strip a leading "export " and any "KEY=" up to the first '='
                _val="${_val#export }"
                if [[ "$_val" == *"="* ]]; then
                    # only strip prefix if it looks like AWS_...=
                    case "$_val" in
                        AWS_*=*) _val="${_val#*=}" ;;
                    esac
                fi
                # Strip surrounding single/double quotes and CR/whitespace
                _val="${_val%$'\r'}"
