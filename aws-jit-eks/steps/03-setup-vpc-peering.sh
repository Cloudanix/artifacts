...
if [[ -n "$PEERING_ID" && "$PEERING_ID" != "None" ]]; then
    ...
    PEERING_ID=$(aws ec2 create-vpc-peering-connection \
        ...
        --tag-specifications "ResourceType=vpc-peering-connection,Tags=[{Key=Name,Value=cdx-jit-eks-peering},{Key=Purpose,Value=eks-jit},{Key=created_by,Value=cloudanix}]" \
