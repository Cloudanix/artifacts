...
if [[ -n "$PEERING_ID" && "$PEERING_ID" != "None" ]]; then
    ...
    PEERING_ID=$(aws ec2 create-vpc-peering-connection \
        ...
        $PEER_REGION_FLAG \
        --tag-specifications "ResourceType=vpc-peering-connection,Tags=[{Key=Name,Value=cdx-jit-eks-peering-${EKS_VPC_ID}},{Key=Purpose,Value=eks-jit},{Key=Environment,Value=Prod},{Key=Created_by,Value=Cloudanix},{Key=purpose,Value=jit_k8s}]" \
